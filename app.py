...
# CSS + UI/UX upgrade live in styles/theme_css.py (out of app.py). It stays a
# Python function because Streamlit can't <link> a stylesheet and the CSS is
# theme-templated. Edit ALL visual styling in styles/theme_css.py, not here.
from styles.theme_css import build_css as _css
...
def login_view():
    ...
    with mid, st.container(key="loginbox"):
        st.markdown(
        ...
        st.markdown(
            f'<div class="dbdot">{cmis_html} &nbsp;·&nbsp; App DB {"🟢" if app_ok else "🔴"}</div>',
            unsafe_allow_html=True,
        )
...
def _batch_day_footprint(raw_df: pd.DataFrame) -> dict[tuple, set[str]]:
    """Map each (date, batch_code) -> the full set of 30-min grid slots that
    batch occupies across the WHOLE day, computed from the RAW (unmerged,
    unfiltered) CMIS rows.

    This is the batch's true footprint. A class can only be delivered if this
    ENTIRE footprint is free -- not just whichever fragment of it happens to
    survive merging. CMIS sometimes splits one real class into rows that don't
    all merge into a single span (e.g. the run breaks at a training boundary,
    or only the tail 30-min row carries the Mock-Interview alias), which is
    exactly how a 03:30-05:30 block leaked through as a lone free 05:00-05:30
    tail. Building the footprint from the raw rows closes that gap.
    """
    fp: dict[tuple, set[str]] = {}
    if raw_df.empty:
        return fp
    date_col = "_date" if "_date" in raw_df.columns else "s_date"
    for _, r in raw_df.iterrows():
        key = (r.get(date_col), r.get("batch_code"))
        fp.setdefault(key, set()).update(_grid_slots_covered({str(r["slot_time"])}))
    return fp


def _keep_blocks_fully_free(
    merged_df: pd.DataFrame,
    free_slots: set[str],
    footprint: dict[tuple, set[str]] | None = None,
) -> pd.DataFrame:
    """Keep only MERGED session blocks whose class can actually be delivered in
    free time.

    A real class is a multi-slot block (e.g. a 2-hour Mock Interview stored as
    four back-to-back 30-min CMIS rows -> one '03:30 PM - 05:30 PM' row after
    _merge_consecutive). It can only be delivered if EVERY 30-min grid slot the
    class occupies is free.

    When `footprint` (from _batch_day_footprint on the RAW rows) is given, a
    block is judged on its BATCH's full-day footprint, so a class whose rows
    split around training -- leaving only a free tail fragment after merging --
    is still correctly dropped. Without a footprint, it falls back to the
    merged block's own span.
    """
    if merged_df.empty:
        return merged_df

    date_col = "_date" if "_date" in merged_df.columns else "s_date"

    def _deliverable(row) -> bool:
        covered = None
        if footprint is not None:
            covered = footprint.get((row.get(date_col), row.get("batch_code")))
        if not covered:
            covered = _grid_slots_covered({str(row["slot_time"])})
        return bool(covered) and covered.issubset(free_slots)

    keep = merged_df.apply(_deliverable, axis=1)
    return merged_df[keep].copy()
...
def _calendar_wizard_tab(user, role):
    ...
    if not mi_candidates.empty:
        ...
        # Merge the raw 30-min CMIS rows into whole classes FIRST, then keep a
        # class only if its ENTIRE span is free -- a 2-hour block half-covered
        # by training can't be delivered and must not appear as a lone free
        # tail slot. Blocks the user has already acted on (any status) stay
        # visible even if no longer free, so a revisitable Not-Selected one
        # doesn't vanish.
        mi_footprint = _batch_day_footprint(mi_candidates)
        merged = _merge_consecutive(mi_candidates)
        fully_free = _keep_blocks_fully_free(merged, set(mi_free_slots), mi_footprint)
        acted = merged[
            merged["slot_time"].apply(
                lambda s: bool(_grid_slots_covered({str(s)}) & mi_acted_slots)
            )
        ]
        mi_display = pd.concat([fully_free, acted]).drop_duplicates(
            subset=["slot_time", "batch_code"]
    ...
    if not all_day.empty:
        # Scope to this person's aligned faculty (and not their own rows)
        # first, but DON'T pre-filter by free slot here -- merge whole classes
        # first, then keep only blocks whose full span is free (below), so a
        # multi-slot class overlapping training isn't shown on just its one
        # free tail slot.
    ...
    if not eval_candidates.empty:
        eval_footprint = _batch_day_footprint(eval_candidates)
        merged_eval = _merge_consecutive(eval_candidates)
        eval_display = _keep_blocks_fully_free(
            merged_eval, set(eval_free_slots), eval_footprint)

    if eval_display.empty:
...
def _util_scrollbar_css():
    ...
    st.markdown(
        ...
        .util-scroll{width:100%;overflow:auto;border:1px solid #e3e7ec;
            ...
            background:#ffffff;max-height:70vh;}
