...
def build_css(t: dict, name: str = "light") -> str:
    ...
    <style>
      ...
      /* ---------------------------------------------------------- */
      /* login_view() now wraps its content in st.container(key="loginbox").
         An older build emitted an empty <div class="login-wrap"> that floated
         as a stray box; if any cached markup still shows it, keep it hidden. */
      .login-wrap {{ display:none !important; }}

      /* Give the login column some top breathing room. */
      [data-testid="stAppViewContainer"] .main .block-container {{ padding-top:4vh; }}

      /* Brand header on the login screen: give it presence. */
      .brandbar {{
      ...
      /* All login styling below is scoped to .st-key-loginbox — a keyed
         container login_view() wraps the form in — so it never touches the
         manager/MI/claim forms elsewhere in the app. */

      /* Field labels: smaller, quieter, uppercase micro-labels = app feel. */
      .st-key-loginbox [data-testid="stTextInput"] label p,
      .st-key-loginbox .stTextInput label p {{
        font-size:.72rem !important; font-weight:700 !important;
        letter-spacing:.06em; text-transform:uppercase;
        color:{t['muted']} !important;
      }}
      /* Inputs: taller, rounder, with a clear teal focus glow. */
      .st-key-loginbox .stTextInput input {{
        min-height:48px !important; border-radius:12px !important;
        font-size:.95rem !important;
      }}
      /* Primary "Sign in" button: full-width gradient pill with lift. */
      .st-key-loginbox .stFormSubmitButton > button {{
      ...
      .st-key-loginbox .stFormSubmitButton > button * {{ color:#fff !important; }}
      .st-key-loginbox .stFormSubmitButton > button:hover {{
        transform:translateY(-1px);
        box-shadow:0 12px 28px {t['accent']}5c !important; filter:brightness(1.03);
      }}
      .st-key-loginbox .stFormSubmitButton > button:active {{ transform:translateY(0) scale(.99); }}

      /* The "Change password" expander: subtle secondary look (login only). */
      .st-key-loginbox [data-testid="stExpander"] {{
        border:1px dashed {t['border']} !important;
        background:transparent !important; border-radius:12px !important;
        margin-top:6px;
      }}
      .st-key-loginbox [data-testid="stExpander"] summary {{ font-weight:600; }}
