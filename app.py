...
def dashboard():
    ...
    if role == "admin":
        ...
        made = st.tabs(["📋  Sessions", "🧭  My Alignment", "📅  Calendar"])
        with made[0]:
            _sessions_tab(user, role)
        with made[1]:
            _my_core_tab(user)
...
def _email_health_tab():
    ...
    view = report.rename(columns={
        ...
        "role": "Role", "suggested_cmis_email": "Suggested CMIS email",
    }).drop(columns=["matches_cmis"])
    st.dataframe(view, use_container_width=True, hide_index=True)

    n_fixable = report["suggested_cmis_email"].notna().sum()
    n_unfixable = len(report) - n_fixable
    st.caption(
        f"{n_fixable} have an unambiguous suggested match; "
        f"{n_unfixable} need a manual look (no unique CMIS prefix match)."
    )
    ...
    with st.expander("📋  Generate fix SQL (review before running — nothing here executes automatically)"):
        ...
        st.caption(
            ...
            "(Anudip_AE_Team, 128.199.28.53) — not CMIS. Review each line "
            "before running; a suggestion is based on a matching mailbox "
            "prefix and could be wrong if two people share one."
        )
...
def _sessions_table(sessions, core_ae_email, date_from, date_to, role, user_email):
    ...
    def _slot_start_ts(r) -> pd.Timestamp:
        raw = r.get("slot_time")
        s = str(raw).split("-")[0].strip() if raw is not None else ""
        t = pd.to_datetime(s, format="%I:%M %p", errors="coerce")
        if pd.isna(t):
            # CMIS sometimes drops the space: "07:30PM"
            t = pd.to_datetime(s, errors="coerce")
        day = pd.Timestamp(r["_date"])
        if pd.isna(t):
            return day + pd.Timedelta(hours=23, minutes=59)  # unparseable -> end of day
        return pd.Timestamp.combine(day.date(), t.time())

    df["_ts"] = df.apply(_slot_start_ts, axis=1)
