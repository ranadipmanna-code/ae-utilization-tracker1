...
def _css(t: dict, name: str = "light") -> str:
    ...
    <style>
      ...
      /* Streamlit's own sidebar collapse/expand control -- the pale default
         ...
         Streamlit changes the test-id of this control between versions, so we
         cast a wide net: the known test-ids AND any header/toolbar button
         whose aria-label mentions the sidebar. Fill it teal and force the
         glyph (svg OR material-icon font) to the on-accent colour. */
...
def dashboard():
    ...
    with st.sidebar:
        ...
        # type="primary" so it renders as a filled, readable button (the CSS
        # hook approach didn't take effect in this Streamlit version).
