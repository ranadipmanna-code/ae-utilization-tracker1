...
def _render_mi_cards(df: pd.DataFrame, user_email: str, key_prefix: str) -> bool:
    ...
    Each row is entirely self-contained inside its own @st.fragment (see
    _render_mi_row_control) -- the card display, the status dropdown, the
    remarks box and the Save button are ALL rendered there, and that
    fragment re-fetches its OWN current status on every rerun instead of
    reading a snapshot computed here. That is what removes the "loading,
    then blank, then loading again" -- previously Save had to force a
    whole-PAGE rerun (st.rerun(scope="app")) just so the Available/Mine
    pill above the controls could refresh, which tore down and rebuilt the
    entire Calendar-wizard day. Now the fragment can refresh itself alone.

    Returns False always now (kept for the caller's signature -- saving no
    longer bubbles a "some card changed" flag up through here since each
    fragment's own rerun already reflects its new state immediately).
    """
    if df.empty:
        return False
    ...
    for trainer, grp in d.groupby("Trainer", sort=False):
        ...
        for _, r in grp.iterrows():
            ...
            row_key = f"{key_prefix}_{r['_date']}|{r['slot_time']}|{b}"
            _render_mi_row_control(
                row_key=row_key,
                date_=r["_date"],
...
def _render_mi_row_control(
    ...
    """One Mock Interview card, fully self-contained: the card display
    (Available/Mine/locked pill), the status dropdown, the optional remarks
    box, and Save -- all in one fragment.

    Re-fetches its OWN current status/owner/remarks on every fragment
    rerun (via db.get_mock_interview_assignments, cached 45s so repeat
    fragments on the same day reuse one cache entry instead of one query
    each) rather than trusting a value the caller computed before this
    fragment ever ran. That is what lets Save use a fragment-scoped
    st.rerun() instead of st.rerun(scope="app") -- the pill above the
    controls lives in THIS fragment now, so refreshing just this fragment
    is enough to show the new status. No more whole-page teardown/rebuild
    on every save.
    """
    def _txt(v) -> str:
        if v is None or (isinstance(v, float) and pd.isna(v)):
    ...
    existing = db.get_mock_interview_assignments(None, date_, date_)
    status, owner, saved_remarks = "Not Selected", None, ""
    if not existing.empty:
        bc = batch_code or ""
        match = existing[
            (existing["slot_time"].astype(str) == str(slot_time))
            & (existing["batch_code"].fillna("").astype(str) == str(bc))
        ]
        if not match.empty:
            row0 = match.iloc[-1]
            status = str(row0["status"])
            owner = row0["extended_ae_email"]
            saved_remarks = row0.get("remarks") or ""
    ...
    day_lbl = pd.to_datetime(date_).strftime("%a, %d %b")
    sub_bits = [duration_text, f"<b>{_txt(batch_code)}</b>"]
    for extra in (_txt(c_alias), _txt(program_name)):
        if extra:
            sub_bits.append(extra)
    sub_line = " \u00b7 ".join(bit for bit in sub_bits if bit and bit != "<b></b>")

    # Card background must agree with the pill above -- a row saved as
    # Not Selected/Rejected always gets an owner (upsert writes it
    # regardless of status), but that shouldn't paint the card teal
    # "mine" when the pill next to it says "Available".
    ...
    with cA:
        st.markdown(
            f"""<div class="scard {card_cls} scard-mi">
              <div class="scard-top">\U0001F551 {day_lbl} &nbsp;\u00b7&nbsp; {_txt(slot_time)} {who}</div>
    ...
    with c3:
        if st.button("\U0001F4BE Save", key=f"{row_key}_save", use_container_width=True):
            try:
                ...
                return
            # Only the MI assignment/claim + pool caches change on this write.
            # Clearing just those (instead of the old clear_app_caches()
            # sledgehammer, which also dumped the member calendar, CMIS session
            # pull and evaluation caches) keeps the rest of the day warm.
            db.clear_mock_interview_caches()
            st.toast(f"Saved: {sel}.", icon="💾")
            # Fragment-scoped rerun only -- the pill above lives in THIS same
            # fragment now, so there's no need to rerun the whole app just to
            # refresh it. This is what removes the full-page blank/reload.
            st.rerun()
