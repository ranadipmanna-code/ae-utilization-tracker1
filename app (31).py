...
def _render_mi_cards(df: pd.DataFrame, user_email: str, key_prefix: str) -> bool:
    ...
    if submitted:
        ...
        n_changed = 0
        conflicts = []
        # A newly-Selected Mock Interview may not overlap Training, an
        # Evaluation, another MI, or a pick accepted earlier in THIS batch --
        # NOT EVEN at the exact same time as a different session. MI cards
        # only ever belong to an Extended AE, so the role is fixed.
        ...
        for row, row_key, orig_status, orig_remarks in sorted(pending, key=_mi_dur):
            ...
            if sel in CLAIMED:
                ...
                if day not in _busy_cache:
                    busy0 = list(db.get_busy_ranges(user_email, _mi_role, day))
                    _busy_cache[day] = busy0
                busy = _busy_cache[day]
                # If this exact session was ALREADY claimed in the DB, its own
                # range is in `busy`; drop it once so it doesn't self-collide.
                # A different session at the same time stays and will clash.
                own_busy = list(busy)
                if orig_status in CLAIMED:
                    _remove_own_range(own_busy, row["slot_time"])
                clash, ns, ne, gap, need = _slot_conflict(own_busy, row["slot_time"])
                if clash:
                    d_lbl = pd.to_datetime(day).strftime("%a, %d %b")
                    tr_nm = (str(row.get("f_name") or "") + " " + str(row.get("l_name") or "")).strip() \
                        or "this interview"
                    conflicts.append(
                        f"\u2022 {d_lbl} \u00b7 {row['slot_time']} \u2014 {tr_nm}  "
                        f"({_conflict_reason(gap, need)})"
                    )
                    continue
                # Accepted -- reserve its range so later picks in this batch
                # (and repeats of the same time) see it as busy.
                if ne is not None:
                    busy.append((ns, ne))
            # ------------------------------------------------------------------
            try:
...
def _sessions_table(sessions, core_ae_email, date_from, date_to, role, user_email, key_prefix=""):
    ...
    if saved:
        if not pending:
            ...
            # both slip through, and two DIFFERENT sessions at the SAME time
            # can't both save either.
            _busy_cache: dict = {}
            ...
            for key, (new_status, r) in sorted(pending.items(), key=_dur):
                ...
                # earlier in this same Save, NOT EVEN at the identical time.
                # Un-selecting never conflicts.
                if new_status in CLAIMED:
                    day = r["_date"]
                    if day not in _busy_cache:
                        _busy_cache[day] = list(db.get_busy_ranges(user_email, role, day))
                    busy = _busy_cache[day]
                    # If THIS session was already claimed, its own sub-slot
                    # ranges are in `busy`; drop them so it doesn't self-clash.
                    # A different session at the same time stays and clashes.
                    own_busy = list(busy)
                    if str(r.get("status")) in CLAIMED:
                        for m_slot in members:
                            _remove_own_range(own_busy, m_slot)
                        _remove_own_range(own_busy, r["slot_time"])
                    clash, ns, ne, gap, need = _slot_conflict(own_busy, r["slot_time"])
                    ...
                    # Accepted -- reserve every sub-slot so later picks in this
                    # same batch (and repeats of the same time) see it as busy.
                    for m_slot in members:
                        ms = _slot_start_minutes(str(m_slot))
                        me = _slot_end_minutes(str(m_slot))
                        if me is not None:
                            busy.append((ms, me))
                    if not members and ne is not None:
                        busy.append((ns, ne))
                # --------------------------------------------------------------
                for m_slot in members:
