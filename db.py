...
def fetch_sessions_all(week_start: date, week_end: date, limit: int = 5000) -> pd.DataFrame:
    """All CMIS sessions in the window (admin overview + team rollup enrichment)."""
    win = _clamp_to_window(week_start, week_end)
    if win is None:
        return pd.DataFrame()
    week_start, week_end = win
    sql = text(
        f"""
        SELECT s_date, m_code, f_name, l_name, time_duration, day_name, c_alias,
               slot_name, slot_time, batch_code, email_id, program_name,
...
def mirror_batches_exist(checks: list[tuple]) -> set:
    """Given (session_date, batch_code) tuples, return the subset still present
    in the mirror. Used to validate Mock Interview saves: an MI is stored as a
    merged span, not a 30-min slot, so we validate that its batch/date block
    still exists rather than an exact slot_time. Live read, not cached."""
    if not checks:
        return set()
    uniq = list({(str(d), str(b or "")) for (d, b) in checks})
    clauses, params = [], {}
    for i, (d, b) in enumerate(uniq):
        clauses.append(f"(s_date = :d{i} AND COALESCE(batch_code,'') = :b{i})")
        params[f"d{i}"] = d
        params[f"b{i}"] = b
    sql = text(
        f"SELECT DISTINCT s_date, COALESCE(batch_code,'') AS batch_code "
        f"FROM {MIRROR_TABLE} WHERE " + " OR ".join(clauses)
    )
    found = set()
    with mirror_engine().connect() as conn:
        for row in conn.execute(sql, params):
            found.add((str(row.s_date), str(row.batch_code)))
    return found


def mirror_sessions_exist(checks: list[tuple]) -> set:
    """Given a list of (session_date, slot_time, batch_code) tuples, return
    the SUBSET that still exists in the mirror right now.

    Used at SAVE time so a selection can't be written against a session that
    the 30-min CMIS sync has since removed or re-timed. slot_time is part of
    the key on purpose: CMIS can change a session's time, and a claim on the
    OLD time should not silently attach to the new one -- it must be re-picked.

    Returns a set of the tuples that are still valid. Anything not in the
    returned set has changed/vanished and should be rejected by the caller.
    NOT cached -- this must read live mirror state at the moment of save.
    """
    if not checks:
        return set()
    # De-dup and build a single query with OR-ed (date, slot, batch) triples.
    uniq = list({(str(d), str(s), str(b or "")) for (d, s, b) in checks})
    clauses = []
    params = {}
    for i, (d, s, b) in enumerate(uniq):
        clauses.append(
            f"(s_date = :d{i} AND slot_time = :s{i} AND "
            f"COALESCE(batch_code,'') = :b{i})"
        )
        params[f"d{i}"] = d
        params[f"s{i}"] = s
        params[f"b{i}"] = b
    sql = text(
        f"SELECT s_date, slot_time, COALESCE(batch_code,'') AS batch_code "
        f"FROM {MIRROR_TABLE} WHERE " + " OR ".join(clauses)
    )
    found = set()
    with mirror_engine().connect() as conn:
        for row in conn.execute(sql, params):
            found.add((str(row.s_date), str(row.slot_time), str(row.batch_code)))
    return found
...
def get_team_extended_ae_activity(
    ...
    """Session (evaluation) claims, Mock Interview assignments, AND Training
    slots for THIS Core AE's own Extended AE team only — scoped via
    extended_aes_for_core, not system-wide.
    Returns (sessions_df, mi_df, training_df).
    ...
    TRAINING is derived from each Extended AE's OWN CMIS slots (their own
    teaching hours in the mirror), with the resolved slot task merged in.
    A slot counts as Training when its resolved task_type is 'training' (an
    explicit override) OR 'teaching' (the CMIS default for a real course
    module). Unlike evaluations/MI, training is NOT a claim the AE opts into
    — it's their scheduled delivery — so it is ALWAYS shown here regardless
    of any selection state.
    """
    ...
    empty_training = pd.DataFrame(
        columns=["owner_email", "session_date", "slot_time", "module",
                 "batch_code", "trainer_name", "c_alias", "program_name",
                 "time_duration", "task_type"]
    )

    ext_aes = extended_aes_for_core(core_ae_email)
    if not ext_aes:
        return empty_sessions, empty_mi, empty_training

    # --- Evaluation claims: ONE batched, cached query for the whole team
    #     instead of one uncached get_selections_for_role per Extended AE.
    #     On a Core AE with several Extended AEs the old loop fired N
    #     uncached round-trips to the remote appdb on EVERY Streamlit rerun
    #     (every dropdown, every tab switch) -- the main source of lag on
    #     this tab. get_selections_for_emails does it in a single IN (...)
    #     query, cached, so reruns are essentially free. -------------------
    sessions = get_selections_for_emails(
        "extended_ae", tuple(ext_aes), from_date, to_date
    )
    sessions = sessions.copy() if not sessions.empty else empty_sessions
    ...
    # --- Training: each Extended AE's own resolved calendar, kept where the
    #     resolved task is training/teaching. Always shown. ---------------
    train_frames = []
    for ext in ext_aes:
        try:
            cal = resolve_member_calendar(ext, from_date, to_date)
        except Exception:
            cal = pd.DataFrame()
        if cal.empty:
            continue
        cal = cal[cal["task_type"].isin(["training", "teaching"])].copy()
        if cal.empty:
            continue
        cal["owner_email"] = ext
        ...
        keep = ["owner_email", "session_date", "slot_time", "module",
                "batch_code", "trainer_name", "c_alias", "program_name",
                "task_type"]
        for col in keep:
            if col not in cal.columns:
                cal[col] = None
        # get_member_own_slots doesn't select time_duration; leave it blank
        # so the rollup falls back to deriving duration from slot_time.
        cal["time_duration"] = float("nan")
        train_frames.append(cal[keep + ["time_duration"]])

    training = (pd.concat(train_frames, ignore_index=True)
                if train_frames else empty_training)

    return sessions, mi, training
...
def get_busy_ranges(email: str, role: str, day: date) -> list[tuple[int, int]]:
    ...
    # --- Training (own delivery slots) ---
    # Use the RESOLVED task, and count BOTH 'teaching' (CMIS default) and
    # 'training' (explicit override). A slot re-tasked to Training is still
    # time the member is committed, so it must block overlapping Evaluation/
    # MI bookings -- and it must match what the Training sections display,
    # otherwise a slot could show as Training yet still be bookable.
    try:
        cal = resolve_member_calendar(email, day, day)
        if not cal.empty:
            for _, r in cal.iterrows():
                if str(r.get("task_type")) in ("teaching", "training"):
...
# The read caches whose data ORIGINATES in the CMIS mirror (refreshed every
# 30 min by sync_cmis_mirror.py). Clearing just these picks up freshly-synced
# session data on the next rerun WITHOUT dumping the app-DB claim caches or
# any in-flight edit state -- so a background heartbeat can keep the portal
# live without disturbing someone mid-form.
_MIRROR_CACHED = (
    "fetch_sessions_all",
    "fetch_sessions_all_for_faculty",
    "get_member_own_slots",
    "get_members_own_slots",
    "resolve_member_calendar",
    # Team activity blends mirror (training) + app-DB (claims); refresh it so
    # newly-synced training shows up on the heartbeat too.
    "get_team_extended_ae_activity",
)


def clear_mirror_caches() -> None:
    """Invalidate ONLY the CMIS-mirror-derived read caches.

    Called on the app's background heartbeat so freshly-synced session and
    training data appears within one heartbeat, without clearing the app-DB
    claim caches (evaluations / Mock Interviews) or forcing edit widgets to
    reset. Also refreshes the MI pool, which is built from mirror sessions."""
    for name in _MIRROR_CACHED:
        fn = globals().get(name)
        clear = getattr(fn, "clear", None)
        if callable(clear):
            try:
...
def trigger_cmis_sync() -> tuple[bool, str]:
    """Kick off the CMIS -> mirror sync GitHub Action on demand.

    This does NOT sync inside the app -- it POSTs a workflow_dispatch to the
    same `cmis-sync.yml` Action the hourly cron uses, so the pull runs on
    GitHub's runner (keeping the app fully decoupled from live CMIS). The
    mirror is refreshed ~1 minute later; the caller should tell the user to
    Refresh after a short wait.

    Requires a [github] section in st.secrets:
        [github]
        token    = "ghp_..."          # PAT with 'workflow' (or repo) scope
        owner    = "ranadipmanna-code"
        repo     = "ae-utilization-tracker1"
        workflow = "cmis-sync.yml"     # optional; defaults to this
        branch   = "main"              # optional; defaults to main

    Returns (ok, message). Never raises -- the button handler shows the msg.
    """
    import json
    ...
    try:
        ...
        return (False, "GitHub sync isn't configured (missing [github] secrets).")

    token = str(gh.get("token") or "").strip()
    owner = str(gh.get("owner") or "").strip()
    repo = str(gh.get("repo") or "").strip()
    workflow = str(gh.get("workflow") or "cmis-sync.yml").strip()
    branch = str(gh.get("branch") or "main").strip()
    if not (token and owner and repo):
        return (False, "GitHub sync isn't fully configured (need token, owner, repo).")
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if resp.status in (201, 202, 204):
                return (True, "Sync started — newest CMIS sessions will appear "
                              "in about a minute. Press Refresh shortly.")
            return (False, f"GitHub returned status {resp.status}.")
        ...
        try:
            ...
            pass
        if e.code == 401:
            return (False, "GitHub rejected the token (401). Check the PAT.")
        if e.code == 404:
            return (False, "Workflow or repo not found (404). Check owner/repo/workflow.")
        return (False, f"GitHub error {e.code}. {detail}")
