...
def get_cmis_known_emails() -> set[str]:
    ...
    sql = text(f"SELECT DISTINCT email_id FROM {CMIS_VIEW} WHERE email_id IS NOT NULL")
    with cmis_engine().connect() as conn:
        df = pd.read_sql(sql, conn)
    return {str(e).strip().lower() for e in df["email_id"] if str(e).strip()}


def _mailbox_prefix(email: str) -> str:
    """The part before the first '.' or '@' — used to guess a likely CMIS
    match when the app-DB email doesn't match exactly (e.g.
    'pulak.bhattacharya@anudip.org' -> 'pulak')."""
    local = email.split("@", 1)[0]
    return local.split(".", 1)[0].strip().lower()


def email_health_report() -> pd.DataFrame:
    """
    Every app-DB email (from user_roles + core_ae_faculty_map) that has NO
    exact match in CMIS's email_id column, with a best-effort suggested CMIS
    email when exactly one CMIS email shares the same mailbox prefix.

    Columns: source, field, app_email, role, matches_cmis, suggested_cmis_email
    Only rows where matches_cmis is False are returned (the report is a
    to-do list, not a full inventory).
    """
    cmis_emails = get_cmis_known_emails()
    # prefix -> list of real CMIS emails sharing that prefix (for suggestions)
    prefix_map: dict[str, list[str]] = {}
    for e in cmis_emails:
        prefix_map.setdefault(_mailbox_prefix(e), []).append(e)
    ...
    for _, r in roles.iterrows():
        ...
        if not email:
            continue
        rows.append({
            "source": "user_roles", "field": "email",
            "app_email": email, "role": r.get("role"),
        })
    ...
    if not rows:
        return pd.DataFrame(columns=[
            "source", "field", "app_email", "role",
            "matches_cmis", "suggested_cmis_email",
        ])

    out = pd.DataFrame(rows).drop_duplicates(subset=["source", "field", "app_email"])
    out["matches_cmis"] = out["app_email"].str.lower().isin(cmis_emails)

    def _suggest(email: str) -> str | None:
        candidates = prefix_map.get(_mailbox_prefix(email), [])
        # only suggest when unambiguous — one CMIS email shares this prefix
        return candidates[0] if len(candidates) == 1 else None

    out["suggested_cmis_email"] = out["app_email"].apply(_suggest)
    mismatches = out[~out["matches_cmis"]].copy()
    return mismatches.sort_values(["source", "app_email"]).reset_index(drop=True)


def build_email_fix_sql(report: pd.DataFrame) -> str:
    """
    Turn email_health_report() rows that HAVE an unambiguous suggestion into
    ready-to-review UPDATE statements. Returns SQL text for a human to read
    and run themselves in phpMyAdmin — this function never executes anything.
    Rows with no suggestion (ambiguous or truly unmatched) are listed as a
    comment instead, since they need a manual decision.
    """
    lines: list[str] = [
        "-- Generated by email_health_report() — REVIEW BEFORE RUNNING.",
        "-- Run on the APP DB server (Anudip_AE_Team), not CMIS.",
        "",
    ]
    fixable = report[report["suggested_cmis_email"].notna()]
    ...
    for _, r in fixable.iterrows():
        ...
        if r["source"] == "user_roles":
            lines.append(
                f"UPDATE user_roles SET email = '{new}' WHERE email = '{old}';"
            )
        else:  # core_ae_faculty_map
            lines.append(
                f"UPDATE core_ae_faculty_map SET {r['field']} = '{new}' "
                f"WHERE {r['field']} = '{old}';"
            )

    if not unfixable.empty:
        lines.append("")
        lines.append("-- The following had NO unambiguous CMIS match — check manually:")
