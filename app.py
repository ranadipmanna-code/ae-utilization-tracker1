...
def _live_data_heartbeat():
    # First paint just records the baseline time; every subsequent tick means
    # >= _HEARTBEAT_SECS has passed, so refresh the mirror-derived reads.
    now = datetime.now()
...
def dashboard():
    ...
    with st.sidebar:
        ...
        # Keeps mirror-derived data fresh on its own timer (see the fragment
        # above) so the portal stays live between the 30-min CMIS syncs
...
def _render_mi_cards(df: pd.DataFrame, user_email: str, key_prefix: str) -> bool:
    ...
    if submitted:
        ...
        n_changed = 0
        conflicts = []
        # Same guard as the Evaluation save path: a newly-Selected Mock
        # Interview may not overlap Training, an Evaluation, another MI, or a
        # pick accepted earlier in THIS batch. MI cards only ever belong to an
        # Extended AE, so the role is fixed.
        _mi_role = "extended_ae"
        _busy_cache: dict = {}

        def _mi_free_gap(busy_ranges, ns, ne):
        ...
        def _mi_dur(item):
            _row = item[0]
            s = _slot_start_minutes(str(_row["slot_time"]))
            e = _slot_end_minutes(str(_row["slot_time"]))
            return (e - s) if (s is not None and e is not None) else 0

        for row, row_key, orig_status, orig_remarks in sorted(pending, key=_mi_dur):
            ...
            # ---- CONFLICT GUARD (only when turning it INTO a claim) ----------
            if sel in CLAIMED:
                day = row["_date"]
                if day not in _busy_cache:
                    _busy_cache[day] = list(db.get_busy_ranges(user_email, _mi_role, day))
                busy = _busy_cache[day]
                ns = _slot_start_minutes(str(row["slot_time"]))
                ne = _slot_end_minutes(str(row["slot_time"]))
                exact_resave = any(ns == bs and ne == be for (bs, be) in busy)
                clash = False
                if ne is not None and not exact_resave:
                    for (bs, be) in busy:
                        if ns < be and bs < ne:
                            clash = True
                            break
                if clash and not exact_resave:
                    d_lbl = pd.to_datetime(day).strftime("%a, %d %b")
                    tr_nm = (str(row.get("f_name") or "") + " " + str(row.get("l_name") or "")).strip() \
                        or "this interview"
                    gap = _mi_free_gap(busy, ns, ne)
                    need = (ne - ns) if (ns is not None and ne is not None) else 0
                    if gap <= 0:
                        why = "calendar is fully booked in this slot"
                    else:
                        why = f"only {gap} min free here, but this session needs {need} min"
                    conflicts.append(
                        f"\u2022 {d_lbl} \u00b7 {row['slot_time']} \u2014 {tr_nm}  ({why})"
                    )
                    continue
                if ne is not None and not exact_resave:
                    busy.append((ns, ne))
        ...
        if conflicts:
            st.error(
                ...
                "schedule (Training, an Evaluation, or another Mock Interview) "
                "and were **not** saved:\n\n" + "\n".join(conflicts) +
        ...
        if not my_mi.empty:
            ...
            if mi_form.form_submit_button("💾  Save my Mock Interview choices", type="primary"):
                ...
                if undecided:
                    ...
                    for wkey, (row, cur) in sorted(row_meta.items(), key=_rowdur):
                        new_status = st.session_state.get(wkey, cur)
                        if new_status == cur:
                            continue

                        # ---- CONFLICT GUARD (only when claiming) -------------
                    ...
                    if conflicts:
                        st.error(
                            ...
                            "your schedule (Training, an Evaluation, or another Mock "
...
def _sessions_table(sessions, core_ae_email, date_from, date_to, role, user_email, key_prefix=""):
    ...
    if saved:
        if not pending:
            ...
            n = 0
            conflicts = []
            # Busy-ranges per day. Seeded ONCE from the DB (Training + already-
            # saved Evaluations/MIs), then GROWN in-place as each pick in THIS
            # batch is accepted -- so two overlapping picks saved together can't
            # both slip through. That intra-batch gap was the actual bug: the
            # guard only ever compared against the pre-save DB state, so a
            # 5:00-5:30 and a 5:00-6:00 chosen in the same Save both passed.
            _busy_cache: dict = {}

            def _free_gap(busy_ranges, ns, ne):
                """Largest number of free minutes inside [ns, ne) given the
                busy ranges -- 0 means the whole slot is already booked."""
                if ns is None or ne is None or ne <= ns:
                    return 0
                # Walk the window, subtracting every overlapping busy range.
                free = 0
                cursor = ns
                for (bs, be) in sorted(busy_ranges):
                    if be <= cursor or bs >= ne:
            ...
            # Sort the batch so SHORTER sessions are placed first. A 30-min
            # pick and a 2-hour pick that start together can't both fit; taking
            # the shorter one first means the message on the longer one is the
            # accurate "not enough room", not an arbitrary order-of-dict loss.
            def _dur(item):
                _k, (_st, _r) = item
                s = _slot_start_minutes(str(_r["slot_time"]))
                e = _slot_end_minutes(str(_r["slot_time"]))
                return (e - s) if (s is not None and e is not None) else 0

            for key, (new_status, r) in sorted(pending.items(), key=_dur):
                ...
                # Evaluation, or a Mock Interview -- NOR anything accepted
                # earlier in this same Save. Un-selecting never conflicts.
                if new_status in CLAIMED:
                    day = r["_date"]
                    if day not in _busy_cache:
                        _busy_cache[day] = list(db.get_busy_ranges(user_email, role, day))
                    busy = _busy_cache[day]
                    ns = _slot_start_minutes(str(r["slot_time"]))
                    ne = _slot_end_minutes(str(r["slot_time"]))
                    ...
                    if clash and not exact_resave:
                        ...
                        tr_nm = (f"{r.get('f_name') or ''} {r.get('l_name') or ''}".strip()
                                 or "this session")
                        gap = _free_gap(busy, ns, ne)
                        need = (ne - ns) if (ns is not None and ne is not None) else 0
                        if gap <= 0:
                            why = "calendar is fully booked in this slot"
                        else:
                            why = (f"only {gap} min free here, but this session "
                                   f"needs {need} min")
                        conflicts.append(
                    ...
                    # Accepted -- reserve its range so later (longer) picks in
                    # this same batch see it as busy.
                    if ne is not None and not exact_resave:
                        busy.append((ns, ne))
                # --------------------------------------------------------------
    ...
    def _assign_to(role_for_target: str, target_email: str) -> list[str]:
        ...
        for r in sorted(picked, key=_pickdur):
            ...
            if clash and not exact_resave:
                ...
                gap = _gap3(busy, ns, ne)
                need = (ne - ns) if (ns is not None and ne is not None) else 0
                why = ("calendar is fully booked in this slot" if gap <= 0
                       else f"only {gap} min free here, but this session needs {need} min")
                skipped.append(f"\u2022 {d_lbl} \u00b7 {r['slot_time']} \u2014 {tr_nm}  ({why})")
                continue
            if ne is not None and not exact_resave:
                busy.append((ns, ne))
            # --------------------------------------------------------------
