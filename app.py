...
def _render_mi_cards(df: pd.DataFrame, user_email: str, key_prefix: str) -> bool:
    ...
    for trainer, grp in d.groupby("Trainer", sort=False):
        ...
        for _, r in grp.iterrows():
            ...
            # Render the interactive control inside a fragment. This is what
            # kills the "double loading":
            #   * Changing the status dropdown used to rerun the WHOLE
            #     Calendar-wizard day (member calendar + CMIS pull +
            #     evaluation cards all re-executing) just to reveal/hide the
            #     remarks box -- that was the first loading.
            #   * The fragment reruns ONLY this card's own block on both the
            #     dropdown change and the Save, so neither touches the rest of
            #     the page. Combined with the scoped cache clear in the save
            #     handler, Save no longer triggers a second full rebuild.
...
def _render_mi_row_control(
    ...
    """One Mock Interview card's status dropdown + optional remarks + Save,
    isolated in a fragment so a dropdown change or a save reruns ONLY this
    block -- not the whole Calendar-wizard day. See _render_mi_cards for why.

    ``row`` is a plain dict (r.to_dict()) so the fragment never holds a live
    pandas row reference across reruns.
    """
    ...
    with c3:
        if st.button("\U0001F4BE Save", key=f"{row_key}_save", use_container_width=True):
            ...
            # Rerun the WHOLE page once so the card's Available/Mine pill and
            # the day summary reflect the new status. scope="app" is required
            # here because the pill lives outside this fragment; without it a
            # fragment-scoped rerun would refresh only the control, not the
            # card above it.
            st.rerun(scope="app")
...
def _merge_rollup_sessions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["_sort"] = pd.to_datetime(d["_start"], format="%I:%M %p", errors="coerce")
    d = d.sort_values(
        ["owner_email", "session_date", "batch_code", "module", "_sort"]
    ).reset_index(drop=True)
...
def _team_rollup(core_ae_email, week_start, week_end):
    ...
    if sel.empty:
        ...
        return
    claimed = sel[sel["status"].isin(list(CLAIMED) + ["Choosing"])].copy()
    if claimed.empty:
        st.caption("No Extended AE selections yet for this week.")
        return

    # ---- enrich with CMIS details (trainer, module alias, programme,
    # duration) -- the raw selection table only stores owner/date/slot/
    # module-code/batch/status, so on its own it can't show "all the
    # details". Joined here on (date, slot_time, batch_code, module=m_code),
    # which is exactly what upsert_selection_for_role wrote the row with. ---
    claimed["session_date"] = pd.to_datetime(claimed["session_date"]).dt.date
    ...
    if not cmis.empty:
        ...
        claimed = claimed.merge(
            lookup, on=["session_date", "slot_time", "batch_code", "module"], how="left",
        )
    else:
        claimed["Trainer"] = ""
        claimed["c_alias"] = ""
        claimed["program_name"] = ""
        claimed["time_duration"] = None
    ...
    for owner, grp in merged.groupby("owner_email", sort=False):
        ...
        for _, r in grp.iterrows():
            ...
            for extra in (_txt(r.get("Trainer")), _txt(r.get("c_alias")), _txt(r.get("program_name"))):
                if extra:
                    sub_bits.append(extra)
            sub_line = " \u00b7 ".join(b for b in sub_bits if b and b != "<b></b>")
            ...
            st.markdown(
                f"""<div class="scard {card_cls}">
                  <div class="scard-top">\U0001F551 {day_lbl} &nbsp;\u00b7&nbsp; {_txt(r.get('slot_time'))}
                    <span class="pill {pill_cls}">{_txt(status)}</span></div>
                  <div class="scard-sub">{sub_line}</div>
                </div>""",
                unsafe_allow_html=True,
            )
